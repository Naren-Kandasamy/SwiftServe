...
- Task assignment — assign staff members to incidents with acknowledgement tracking
...
- Role-based views — Security sees security incidents, Medical sees health incidents, Management sees all
...
/venues/{venueId}/
  /config/
    ...
    staffRoster/{userId}: { name, role, fcmToken, hasFirstAid }
  /alerts/{alertId}: Alert object
  /incidents/{incidentId}: Incident object
  /messages/{incidentId}/{messageId}: { senderId, text, timestamp, type }
```
